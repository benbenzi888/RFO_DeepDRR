...
class ScatterNet():
    ...
    def add_scatter(self, input_image, camera):
        ...
        uselen = int(np.power(2,np.ceil(np.log2(maxlen))))
        max = np.max(input_image)
        tensor_list = []
        cuts = []
        for i in range(0, input_image.shape[0]):
            image = resize(input_image[i, :, :] / max, [int(np.round(input_image.shape[1] * scale_factor)), int(np.round(input_image.shape[2] * scale_factor))]) * max * 256 / 5.5
        ...
        scatter = np.array(output.cpu().data)
